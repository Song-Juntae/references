...
* 제 1강 통계학의 정의
    ...
    * 데이터화 되는 원자료(raw data)를 설명하기 위한 용어
...
* 제 3강 통계의 기본 개념
    * 통계 개념
        ...
        * -1σ와 1σ 사이에서 전체 자료의 약 68%가 관측이 되고, -2σ와 2σ 사이에서 약 95%가 관측된다
...
* 제 4강 자료분석과 기술 통계
    * 기술통계 자료분석 : 자료의 특성을 표현
        * 1) 자료를 효과적으로 정리
        * 2) 표(table), 그림(chart or graph), 숫자, % 등
        * 3) 너무 자세하거나 혹은 과장되게 표현하는 것은 부적절함
    ...
    * 그래프나 도표를 이용하는 방법
        ...
        * 7) 선 그래프(line plot)
    * 원도표
        * 1) 도수분포(frequency distribution)를 그림으로 나타냄
        * 2) 각 범주 항목(혹은 계급)을 쉽게 이해
        * 3) 항목의 수가 3~6개 정도가 적당함
    * 막대 도표
        * 1) 가장 널리 사용되는 도표
        * 2) 범주의 항목별 그 수로 막대의 높이가 결정
        * 3) 최초 작성 뒤 순서열에 맞춰 재 작성시 이해가 쉬움
    * 히스토그램
        * 1) 연속형 자료에 사용
        * 2) 시작점과 끝점 그리고 막대의 수가 중요함
        * 3) 막대의 수는 sturge 공식에 따라 정함 k = 1 + log2(n)
        * 4) 막대 도표와 다르게 열이 연속형 변수를 나타내므로 왜도(치우침)를 보여준다
    * 줄기와 잎그림
        * 1) 연속형 자료에 사용
        * 2) 자료 요약, 개별 측정치를 동시에 나타낼 수 있음
        * 3) 단 지나치게 자료개수가 많거나, 자릿수가 큰 경우 부적절
    * 상자 그림
        * 1) 자료 집합의 범위와 중앙값을 빠르게 확인
        * 2) 자료에서 얻은 다섯 수치
        * 다섯수치 요약
            * 1) 최소값 : 제 1사분위에서 1.5IQR을 뺀 위치
            * 2) 제 1사분위(Q1) : 25% 위치
            * 3) 제 2사분위(Q2) : 50% 중앙값을 의미
            * 4) 제 3사분위(Q3) : 75% 위치
            * 5) 최대값 : 제 3사분위에서 1.5IQR을 더한 위치
            * IQR(Interquartile range) : 사분위간 범위(Q3-Q1)
            * 이상치(Outlier) : 최소값과 최대값을 넘어가는 위치에 있는 값 
    * 산점도
        * 1) 대응하는 2종류의 데이터의 관계를 가로축과 세로축의 대응점에 점을 찍어 표현
        * 2) 대응하는 2종류의 데이터가 양의 관계, 음의 관계 혹은 아무런 관계가 없는 지 직관적으로 알려줌
        * 3) 일반적으로 상관분석과 회귀분석을 하기 전에 자료 간의 관계를 눈으로 파악하기 위해 사용함
    * 선그래프
        * 1) 반복적인 측정이 있는 자료를 나타내는데 유용
        * 2) 간단하고 파악하기 쉬움 또한 자료가 많은 경우 유용한 도구로 사용됨

     * 수치적 계산에 의한 방법
        * 중심화 경향 : 자료들의 중심을 알아내는 방법
            * 평균(Mean), 중위수(Median), 최빈값(Mode)
        * 산포도 : 자료들이 중심을 기준으로 어떻게 흩어졌는지 알아내는 방법
            * 분산(Variance), 표준편차(Stddev.), 범위(Range), 사분위범위(IQR)
        * 분포도 : 자료들의 좌우대칭정도(왜도)와 뾰족함 정도(첨도)를 알아내는 방법
            * 왜도(Skewness), 첨도(Kurtosis)
        * 자료의 중심, 대표값
            * 1) 평균 : 자료를 구성하는 모든 측정치들을 모두 더한 값을 자료의 크기로 나누어 준 값 
            * 2) 중앙값 : 자료를 크기 오름차순으로 배열했을 때 가운데(50%) 위치하는 값, 이상치에 영향을 받지 않음
            * 3) 최빈값 : 가장 빈도가 많이 나타내는 측정값
        * 자료의 흩어진 정도
            * 1) 편차 : 평균을 중심으로 자료들의 퍼져있는 정도(편차의 합은 0이며, 편차의 평균도 항상 0이다)
            * 2) 분산 : 편차제곱의 합을 자유도(N-1)로 나눈값
            * 3) 사분위수 : 상자그림에서 사용되었던 다섯 수치 요약 참조
